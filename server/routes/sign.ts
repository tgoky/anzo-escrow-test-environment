...
export const signAttestation = async (req: Request, res: Response) => {
  console.log('🛠️ POST /api/sign handler triggered');

  try {
    if (!PRIMUS_APP_SECRET) {
      console.error('PRIMUS_APP_SECRET is not defined');
      return res.status(500).json({ error: 'Server configuration error: Missing PRIMUS_APP_SECRET' });
    }
    ...
    try {
      ...
      return res.status(400).json({
        error: 'Invalid signParams format',
    ...
    return res.status(200).json({ signResult: enhancedSignResult });
  } catch (error) {
